...
/**
 * Factory method. creates cube instance from a string with data.
 */
@Slf4j
public class CubeFactory implements FigureFactory<Cube> {
	 ...
	 */
	private static final Pattern CUBE_PATTERN = Pattern.compile("(\\w+):" + "\\{\\(([+-]?\\d+(?:\\.\\d+)?),"
			+ "([+-]?\\d+(?:\\.\\d+)?)," + "([+-]?\\d+(?:\\.\\d+)?)\\)," + "([+-]?\\d+(?:\\.\\d+)?)\\}");
	...
	public Cube createFigure(final String data) {
		...
		if (!matcher.matches()) {
			log.error("the string didn't matched regex with cube data = {}", data);
			throw new GeometryException("invalid format of string with cube data!");
		}
		...
		Cube cube = new Cube(matcher.group(NAME_GROUP), new Point(matcher.group(NAME_GROUP), x, y, z) {
		}, edgeLength) {
		};
