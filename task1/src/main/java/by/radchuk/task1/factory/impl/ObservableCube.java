...
/**
 * Cube extension. Implements a geometry Cube model in three-dimensional space.
 * Can be stored in a repository-observer. Notifies observer every time when
 * happens any changes to the data
 * 
 * @author Dmitry Radchuk
 */
@Slf4j
@EqualsAndHashCode(callSuper = true)
class ObservableCube extends CheckedCube implements Observable, Observer {
	/**
	 * observer reference.
	 */
	@Setter
	...
	public void notifyObserver() {
		log.debug("Notifying observer of the Cube with id={}" + " new Point id={}", this.getId(),
	...
	public void update(final int id) {
		log.debug("Cube with id={} has been changed", this.getId());
		this.notifyObserver();
	}
	...
	public void setEdgeLength(final double length) {
		...
		log.debug("Cube with id={} edge length has been changed," + " new edge={}", this.getId(), length);
		this.notifyObserver();
	}
}
