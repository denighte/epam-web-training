...
/**
 * factory method. creates Point instance from a string with data.
 */
@Slf4j
public class PointFactory implements FigureFactory<Point> {
	...
	public Point createFigure(final String data) {
		...
		if (!matcher.matches()) {
			log.error("the string didn't matched regex with point data = {}", data);
			throw new GeometryException("invalid format of string with point data!");
		}
